...
percent_sequence = ['B', 'C', 'D','F', 'G', 'H','J', 'K', 'L','N', 'O', 'P','R', 'S', 'T','V', 'W', 'X','Z', 'AA', 'AB']
ownership_sequence = ['GW1','GW2','GW3','Priv','SLWC','SLC2','BGO']
letters_sequence = ['D','E','H','I','L','M','P','Q','T','U','X', 'Y','AB','AC']
...
def adjtop (para,current,old):
    ...
    if (current>old):
        ...
        para.add_run('%')
        para.paragraph_format.left_indent = ds.Inches(1.5)
...
def top5 (letter1,letter2,letter3):
    ...
    if ownership == "Bentall Green Oak":
        i = 5
        while i<9:
...
for ownership in ownership_sequence:
    ...
    if (end == 13 and sheet.range(percent_sequence[z] + "13").value =="No Properties at 100% Occupancy"):
        ...
        full_occ.paragraph_format.left_indent = ds.Inches(1.5)
    ...
    while occ_row <= end:
        ...
        all_occ.paragraph_format.left_indent = ds.Inches(1.5)
    ...
    # we have printed the top 5 vacancy section and the 100% occupancy, cant figure out leasing velocity so we move on to new units signed
    
    # we will however print out the bullet points for velocity so we dont have issues later
    ...
    if (velocity_end == 51 and sheet.range(percent_sequence[z] + "51").value =="No new units signed"):
    ...
    while velocity_row <= velocity_end:
        ...
        all_velocity.paragraph_format.left_indent = ds.Inches(1.5)
        
        velocity_row = velocity_row + 1
    
    
    unit_number = str(int(sheet.range(percent_sequence[z+2]+"81").value))
    ...
    if (lease_end == 82 and sheet.range(percent_sequence[z] + "82").value =="No new units signed"):
    ...
    while lease_row <= lease_end:
        ...
        all_lease.paragraph_format.left_indent = ds.Inches(1.5)
...
document.save('Private Commentaries.docx')

print("see Private Commentaries file at \"U:\python tester folder\" for your commentaries")
